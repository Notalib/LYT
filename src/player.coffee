...
LYT.player = 
  ...
  init: ->
    ...
    jplayer = @el.jPlayer
      ...
      timeupdate: (event) =>
        if event.jPlayer.status.duration - event.jPlayer.status.currentTime < 2
          log.warn "Player: media is 2 seconds from ending and we can'ht handle that - skip ahead now"
          @nextSection true
